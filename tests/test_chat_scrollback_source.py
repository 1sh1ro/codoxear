...
class TestChatScrollbackSource(unittest.TestCase):
    ...
    def test_composer_enter_submits_shift_enter_keeps_newline(self) -> None:
        source = APP_JS.read_text(encoding="utf-8")
        start = source.index('textarea.addEventListener("keydown"')
        end = source.index('window.addEventListener("resize"', start)
        block = source[start:end]
        self.assertIn('if (e.key !== "Enter") return;', block)
        self.assertIn("if (e.isComposing) return;", block)
        self.assertIn("if (e.shiftKey) return;", block)
        self.assertIn("e.preventDefault();", block)
        self.assertIn("form.requestSubmit();", block)
        self.assertNotIn("ctrlKey", block)
        self.assertNotIn("metaKey", block)
