      ...
      const splitShellMode = pageParams.get("split") === "1" && !pageParams.get("pane");
      const paneId = (() => {
        const raw = String(pageParams.get("pane") || "").trim().toLowerCase();
        return raw === "left" || raw === "right" ? raw : "";
      })();
      function splitSessionIdsFromHash() {
        const raw = window.location.hash.startsWith("#") ? window.location.hash.slice(1) : "";
        const params = new URLSearchParams(raw);
        return {
          left: String(params.get("left") || params.get("session") || "").trim(),
          right: String(params.get("right") || "").trim(),
        };
      }
      function urlForMode({ split = false, pane = "", session = undefined, leftSession = undefined, rightSession = undefined } = {}) {
        ...
        if (leftSession !== undefined || rightSession !== undefined) {
          const params = new URLSearchParams();
          if (leftSession) params.set("left", leftSession);
          if (rightSession) params.set("right", rightSession);
          const next = params.toString();
          url.hash = next ? `#${next}` : "";
        } else if (session === undefined) {
          // Preserve the current hash.
        } else if (session) {
          url.hash = `#session=${encodeURIComponent(session)}`;
        } else {
          url.hash = "";
        }
        return url.toString();
      }
      function paneStorageKey(base) {
      ...
      function iconSvg(name) {
        ...
        if (name === "tailscale")
          return `<svg class="icon" viewBox="0 0 24 24" fill="currentColor"><circle cx="7" cy="7" r="2.2"/><circle cx="12" cy="7" r="2.2"/><circle cx="17" cy="7" r="2.2"/><circle cx="7" cy="12" r="2.2"/><circle cx="12" cy="12" r="2.2"/><circle cx="17" cy="12" r="2.2"/><circle cx="7" cy="17" r="2.2"/><circle cx="12" cy="17" r="2.2"/><circle cx="17" cy="17" r="2.2"/></svg>`;
        if (name === "tokens")
          return `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h16"/><path d="M4 12h10"/><path d="M4 17h7"/><path d="M17 14l3 3-3 3"/></svg>`;
        if (name === "play")
        ...
        if (name === "split")
          return `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16"/><path d="M4 18h16"/><path d="M12 6v12"/></svg>`;
        if (name === "right")
      ...
      function renderSplitShell() {
        const root = $("#root");
        root.innerHTML = "";
        const splitSessions = splitSessionIdsFromHash();
        const leftSession = splitSessions.left;
        const rightSession = splitSessions.right;
        const wrap = el("div", { class: "splitShell" });
        const splitTopbar = el("div", { class: "splitShellTopbar" }, [
          el("div", { class: "splitShellTitle", html: `<img class="sidebarLogo" src="/static/codoxear-icon.png" alt="" />Codoxear split` }),
          el("button", { id: "splitRestoreBtn", class: "splitRestoreBtn", type: "button", title: "Restore single window", "aria-label": "Restore single window", html: `${iconSvg("left")}Restore` }),
        ]);
        const panes = el("div", { class: "splitPanes" }, [
          el("iframe", { class: "splitPaneFrame", title: "Left Codoxear conversation", src: urlForMode({ split: false, pane: "left", session: leftSession || "" }) }),
          el("iframe", { class: "splitPaneFrame", title: "Right Codoxear conversation", src: urlForMode({ split: false, pane: "right", session: rightSession || "" }) }),
        ]);
        wrap.appendChild(splitTopbar);
        wrap.appendChild(panes);
        root.appendChild(wrap);
        const restore = $("#splitRestoreBtn");
        if (restore) restore.onclick = () => {
          window.location.href = urlForMode({ split: false, session: leftSession || rightSession || "" });
        };
      }
        ...
        const tailscaleBtn = el("button", {
          id: "tailscaleBtn",
          class: "icon-btn tailscaleBtn",
          title: "Tailscale status",
          "aria-label": "Tailscale status",
          type: "button",
          html: `${iconSvg("tailscale")}<span class="tailscaleCount" id="tailscaleCount"></span>`,
        });
        const tokenUsageBtn = el("button", {
          id: "tokenUsageBtn",
          class: "icon-btn tokenUsageBtn",
          title: "Today's token usage",
          "aria-label": "Today's token usage",
          type: "button",
          html: `${iconSvg("tokens")}<span class="tokenUsageCount" id="tokenUsageCount"></span>`,
        });
        const splitBtn = el("button", {
          id: "splitBtn",
          class: "icon-btn",
          title: paneId ? "Restore single window" : "Split view",
          "aria-label": paneId ? "Restore single window" : "Split view",
          type: "button",
          html: iconSvg(paneId ? "left" : "split"),
        });
        splitBtn.disabled = !paneId;
        ...
        const tailscalePanel = el("div", { id: "tailscalePanel", class: "tailscalePanel", role: "dialog", "aria-label": "Tailscale machines" }, [
          el("div", { class: "tailscalePanelHead" }, [
            el("div", { class: "tailscalePanelTitle", text: "Tailscale" }),
            el("div", { class: "muted tailscaleUpdated", id: "tailscaleUpdated", text: "" }),
          ]),
          el("div", { class: "tailscaleSummary", id: "tailscaleSummary", text: "Loading..." }),
          el("div", { class: "tailscaleMachines", id: "tailscaleMachines" }),
        ]);
        const tokenUsagePanel = el("div", { id: "tokenUsagePanel", class: "tokenUsagePanel", role: "dialog", "aria-label": "Today's token usage" }, [
          el("div", { class: "tokenUsagePanelHead" }, [
            el("div", { class: "tokenUsagePanelTitle", text: "Today tokens" }),
            el("div", { class: "muted tokenUsageUpdated", id: "tokenUsageUpdated", text: "" }),
          ]),
          el("div", { class: "tokenUsageSummary", id: "tokenUsageSummary", text: "Loading..." }),
          el("div", { class: "tokenUsageGrid", id: "tokenUsageGrid" }),
          el("div", { class: "tokenUsageRoots", id: "tokenUsageRoots" }),
          el("div", { class: "tokenUsageSessions", id: "tokenUsageSessions" }),
        ]);
        ...
        root.appendChild(tailscalePanel);
        root.appendChild(tokenUsagePanel);
        ...
        const splitChoiceBackdrop = el("div", { class: "modalBackdrop", id: "splitChoiceBackdrop" });
        const splitChoiceCancel = el("button", {
          id: "splitChoiceCancel",
          class: "icon-btn",
          title: "Cancel",
          "aria-label": "Cancel",
          type: "button",
        ...
        const splitChoiceList = el("div", { class: "splitChoiceList", id: "splitChoiceList" });
        const splitChoiceCurrent = el("div", { class: "muted splitChoiceCurrent", id: "splitChoiceCurrent", text: "" });
        const splitChoice = el("div", { class: "splitChoice", id: "splitChoice", role: "dialog", "aria-label": "Choose conversation for split view" }, [
          el("div", { class: "queueHeader" }, [
            el("div", { class: "title", text: "Choose conversation" }),
            el("div", { class: "actions" }, [splitChoiceCancel]),
          ]),
          splitChoiceCurrent,
          splitChoiceList,
        ]);
        root.appendChild(splitChoiceBackdrop);
        root.appendChild(splitChoice);

        function splitChoiceRows() {
          const currentId = selected;
          const sessions = Array.from(sessionIndex.values())
            .filter((s) => s && s.session_id && s.session_id !== currentId && sessionSelectable(s))
            .sort((a, b) => {
          ...
          splitChoiceList.replaceChildren();
          if (!sessions.length) {
            splitChoiceList.appendChild(el("div", { class: "muted splitChoiceEmpty", text: "No other conversations available." }));
            return;
          }
          for (const s of sessions) {
            const name = sessionDisplayName(s) || shortSessionId(s.session_id);
            const updatedTs = typeof s.updated_ts === "number" && Number.isFinite(s.updated_ts)
              ? s.updated_ts
              : typeof s.start_ts === "number" && Number.isFinite(s.start_ts)
                ? s.start_ts
                : 0;
            const metaBits = [shortSessionId(s.session_id)];
            const cwdBase = baseName(s.cwd);
            if (cwdBase) metaBits.push(cwdBase);
            const branchTxt = typeof s.git_branch === "string" ? s.git_branch.trim() : "";
            if (branchTxt) metaBits.push(branchTxt);
            if (updatedTs) metaBits.push(fmtRelativeAge(Math.max(0, Date.now() / 1000 - updatedTs)));
            const row = el("button", { class: "splitChoiceRow", type: "button" }, [
              el("div", { class: "splitChoiceRowTitle", text: name }),
              el("div", { class: "muted splitChoiceRowMeta", text: metaBits.filter(Boolean).join(" | ") }),
            ]);
            row.onclick = () => {
              if (!selected) return;
              const leftSession = selected;
              window.location.href = urlForMode({ split: true, leftSession, rightSession: s.session_id });
            };
            splitChoiceList.appendChild(row);
          }
        }

        async function showSplitChoice() {
          if (!selected) {
            setToast("select a session first");
            return;
          }
          splitChoiceBackdrop.style.display = "block";
          splitChoice.style.display = "flex";
          splitChoiceCurrent.textContent = `Current session: ${sessionTitleWithId(sessionIndex.get(selected))} (${shortSessionId(selected)})`;
          splitChoiceList.replaceChildren(el("div", { class: "muted splitChoiceEmpty", text: "Loading conversations..." }));
          try {
            await refreshSessions();
          } catch (e) {
            console.error("refreshSessions failed before split chooser", e);
          }
          if (!selected || !sessionIndex.has(selected)) {
            hideSplitChoice();
            setToast("current session is no longer available");
            return;
          }
          splitChoiceCurrent.textContent = `Current session: ${sessionTitleWithId(sessionIndex.get(selected))} (${shortSessionId(selected)})`;
          splitChoiceRows();
        ...
        function hideSplitChoice() {
          splitChoiceBackdrop.style.display = "none";
          splitChoice.style.display = "none";
          splitChoiceList.replaceChildren();
        }

        splitBtn.onclick = () => {
          if (paneId) {
            window.top.location.href = urlForMode({ split: false });
            return;
          }
          void showSplitChoice();
        };
        splitChoiceBackdrop.onclick = () => hideSplitChoice();
        splitChoice.onclick = (e) => e.stopPropagation();
        splitChoiceCancel.onclick = () => hideSplitChoice();
      ...
      function consumePendingUserIfMatches(ev, sessionId = selected) {
        ...
        function setSidebarOpen(open) {
          if (open) {
            ...
            localStorage.setItem(paneStorageKey("codexweb.sidebarOpen"), "1");
          } else {
            document.body.classList.remove("sidebar-open");
            localStorage.removeItem(paneStorageKey("codexweb.sidebarOpen"));
          }
        }
        ...
        function setSidebarCollapsed(collapsed) {
          if (collapsed) {
            ...
            localStorage.setItem(paneStorageKey("codexweb.sidebarCollapsed"), "1");
          } else {
            document.body.classList.remove("sidebar-collapsed");
            localStorage.removeItem(paneStorageKey("codexweb.sidebarCollapsed"));
          }
        }

        function sessionSplitTargetId(dragSessionId) {
          if (selected && selected !== dragSessionId) return selected;
          const fallback = Array.from(sessionIndex.values()).find((item) => {
            return item && item.session_id && item.session_id !== dragSessionId && sessionSelectable(item);
          });
          return fallback ? fallback.session_id : "";
        }

        function hideSessionSplitDrag() {
          if (!sessionSplitDrag) return;
          if (sessionSplitDrag.preview && sessionSplitDrag.preview.parentNode) sessionSplitDrag.preview.remove();
          document.body.classList.remove("session-split-dragging", "session-split-armed");
          if (sessionSplitDrag.card) sessionSplitDrag.card.classList.remove("draggingSplit");
          try {
            if (sessionSplitDrag.card && sessionSplitDrag.pointerId != null) {
              sessionSplitDrag.card.releasePointerCapture(sessionSplitDrag.pointerId);
            }
          } catch (_) {}
          sessionSplitDrag = null;
        }

        function updateSessionSplitPreview(e) {
          if (!sessionSplitDrag) return;
          const dx = e.clientX - sessionSplitDrag.startX;
          const dy = e.clientY - sessionSplitDrag.startY;
          const horizontalDrag = Math.abs(dx) > 8 && Math.abs(dx) > Math.abs(dy) * 1.15;
          if (!sessionSplitDrag.started) {
            if (!horizontalDrag) return;
            const preview = el("div", { class: "sessionSplitPreview" }, [
              el("div", { class: "sessionSplitPreviewTitle", text: sessionSplitDrag.title }),
              el("div", { class: "muted sessionSplitPreviewMeta", text: "Drag outside the sidebar to split" }),
            ]);
            document.body.appendChild(preview);
            sessionSplitDrag.preview = preview;
            sessionSplitDrag.started = true;
            sessionSplitDrag.card.classList.add("draggingSplit");
            document.body.classList.add("session-split-dragging");
          }
          sessionSplitDrag.preview.style.transform = `translate3d(${e.clientX + 12}px, ${e.clientY + 10}px, 0)`;
          const armed = e.clientX > sessionSplitDrag.thresholdX && Math.abs(dx) > 20 && Math.abs(dx) > Math.abs(dy) * 1.15;
          sessionSplitDrag.armed = armed;
          document.body.classList.toggle("session-split-armed", armed);
        }

        function startSessionSplitDrag(card, s, e) {
          if (!useDesktopSessionActions()) return false;
          if (!s || !s.session_id || !sessionSelectable(s)) return false;
          if (e.button !== 0 || e.pointerType !== "mouse") return false;
          const target = e.target instanceof Element ? e.target.closest("button, a, input, textarea, select, [role='button']") : null;
          if (target) return false;
          const sidebarRect = sidebar.getBoundingClientRect();
          sessionSplitDrag = {
            card,
            sessionId: s.session_id,
            title: sessionDisplayName(s) || shortSessionId(s.session_id),
            startX: e.clientX,
            startY: e.clientY,
            thresholdX: sidebarRect.right + 24,
        ...
        function moveSessionSplitDrag(e) {
          if (!sessionSplitDrag) return;
          updateSessionSplitPreview(e);
          if (sessionSplitDrag.started) e.preventDefault();
          if (!sessionSplitDrag.armed) return;
          const rightSession = sessionSplitDrag.sessionId;
          const leftSession = sessionSplitTargetId(rightSession);
          suppressSessionClickUntil = Date.now() + 700;
          hideSessionSplitDrag();
          if (!leftSession) {
            setToast("select another session first");
            return;
          }
          window.location.href = urlForMode({ split: true, leftSession, rightSession });
        }

        function finishSessionSplitDrag() {
          if (!sessionSplitDrag) return;
          const suppressClick = !!sessionSplitDrag.started;
          hideSessionSplitDrag();
          if (suppressClick) suppressSessionClickUntil = Date.now() + 250;
        }
              ...
              try {
                ...
                if (selected === s.session_id) {
                  selected = null;
                   ...
                   historyCursor = null;
                   turnOpen = false;
                   localStorage.removeItem(paneStorageKey("codexweb.selected"));
                   setSessionHash("");
             ...
             if (swipeActions) {
               ...
               card.onclick = () => {
                 ...
                 card.addEventListener("pointerdown", (e) => {
                   startSessionSplitDrag(card, s, e);
                 });
                 card.addEventListener("pointermove", moveSessionSplitDrag);
                 card.addEventListener("pointerup", finishSessionSplitDrag);
                 card.addEventListener("pointercancel", finishSessionSplitDrag);
	               card.onclick = () => {
        ...
        async function openSession(sessionId, { useCache = true } = {}) {
          ...
          selected = sessionId;
          localStorage.setItem(paneStorageKey("codexweb.selected"), sessionId);
        ...
        function updateHarnessBtnState() {
          ...
          splitBtn.disabled = paneId ? false : !selected;
        ...
        tailscaleBtn.onclick = (e) => {
          e.preventDefault();
          e.stopPropagation();
          if (tailscalePanel.style.display === "block") hideTailscalePanel();
          else showTailscalePanel();
        };
        tailscalePanel.onclick = (e) => e.stopPropagation();
        tokenUsageBtn.onclick = (e) => {
          e.preventDefault();
          e.stopPropagation();
          if (tokenUsagePanel.style.display === "block") hideTokenUsagePanel();
          else showTokenUsagePanel();
        };
        tokenUsagePanel.onclick = (e) => e.stopPropagation();
        document.addEventListener("click", () => {
          if (tailscalePanel.style.display === "block") hideTailscalePanel();
          if (tokenUsagePanel.style.display === "block") hideTokenUsagePanel();
        });
        window.addEventListener("resize", () => {
          positionTailscalePanel();
          positionTokenUsagePanel();
        });
        ...
        function tailscaleMachineName(machine) {
          const host = String((machine && machine.hostname) || "").trim();
          if (host) return host;
          const dns = String((machine && machine.dns_name) || "").trim();
          if (dns) return dns.split(".")[0] || dns;
          const ips = machine && Array.isArray(machine.ips) ? machine.ips : [];
          return ips.length ? String(ips[0]) : "unknown";
        }

        function tailscaleRelativeTime(ts) {
          const v = Number(ts || 0);
          if (!Number.isFinite(v) || v <= 0) return "";
          const age = Math.max(0, Date.now() / 1000 - v);
        ...
        function fmtTokenCount(n, { compact = false } = {}) {
          const v = Number(n || 0);
          if (!Number.isFinite(v)) return "0";
          if (!compact) return Math.round(v).toLocaleString();
          const abs = Math.abs(v);
          if (abs >= 1000000000) return `${(v / 1000000000).toFixed(abs >= 10000000000 ? 1 : 2)}B`;
          if (abs >= 1000000) return `${(v / 1000000).toFixed(abs >= 10000000 ? 1 : 2)}M`;
          if (abs >= 1000) return `${(v / 1000).toFixed(abs >= 10000 ? 1 : 2)}K`;
          return String(Math.round(v));
        }

        function tokenUsageSessionName(row) {
          const name = String((row && row.name) || "").trim();
          if (!name) return "session";
          return name.replace(/^rollout-/, "").replace(/\.jsonl$/, "");
        }

        function positionTailscalePanel() {
          if (tailscalePanel.style.display !== "block") return;
          const rect = tailscaleBtn.getBoundingClientRect();
          const top = Math.min(window.innerHeight - 12, rect.bottom + 8);
          tailscalePanel.style.top = `${top}px`;
          tailscalePanel.style.left = "12px";
          tailscalePanel.style.right = "auto";
          const w = tailscalePanel.offsetWidth || 340;
          const left = Math.max(12, Math.min(window.innerWidth - 12 - w, rect.right - w));
          tailscalePanel.style.left = `${left}px`;
        }

        function positionTokenUsagePanel() {
          if (tokenUsagePanel.style.display !== "block") return;
          const rect = tokenUsageBtn.getBoundingClientRect();
          const top = Math.min(window.innerHeight - 12, rect.bottom + 8);
          tokenUsagePanel.style.top = `${top}px`;
          tokenUsagePanel.style.left = "12px";
          tokenUsagePanel.style.right = "auto";
          const w = tokenUsagePanel.offsetWidth || 380;
          const left = Math.max(12, Math.min(window.innerWidth - 12 - w, rect.right - w));
          tokenUsagePanel.style.left = `${left}px`;
        }

        function hideTailscalePanel() {
          tailscalePanel.style.display = "none";
        }

        function hideTokenUsagePanel() {
          tokenUsagePanel.style.display = "none";
        }

        function showTailscalePanel() {
          tailscalePanel.style.display = "block";
          positionTailscalePanel();
          void loadTailscaleStatus({ immediate: true });
        }

        function showTokenUsagePanel() {
          tokenUsagePanel.style.display = "block";
          positionTokenUsagePanel();
          void loadTokenUsageToday({ immediate: true });
        }

        function renderTailscaleStatus(data) {
          const ok = !!(data && data.ok);
          const machines = data && Array.isArray(data.machines) ? data.machines : [];
          const online = ok ? Number(data.online_count || 0) : 0;
          const total = ok ? Number(data.machine_count || machines.length || 0) : 0;
          const backendState = String((data && data.backend_state) || "").trim();
          tailscaleBtn.classList.toggle("active", ok && online > 0);
          tailscaleBtn.classList.toggle("error", !ok || (backendState && backendState.toLowerCase() !== "running"));
          tailscaleBtn.title = ok ? `Tailscale ${online}/${total} online` : `Tailscale unavailable: ${String((data && data.error) || "unknown error")}`;
          tailscaleBtn.setAttribute("aria-label", tailscaleBtn.title);
          const countEl = $("#tailscaleCount");
          if (countEl) {
            countEl.textContent = ok && total ? String(online) : "!";
            countEl.style.display = "inline-flex";
          }
          const summaryEl = $("#tailscaleSummary");
          const updatedEl = $("#tailscaleUpdated");
          const listEl = $("#tailscaleMachines");
          if (updatedEl) updatedEl.textContent = data && data.updated_ts ? tailscaleRelativeTime(data.updated_ts) : "";
          if (summaryEl) {
            if (ok) {
              const parts = [`${online}/${total} online`];
              if (backendState) parts.push(backendState);
              if (data.health && Array.isArray(data.health) && data.health.length) parts.push(`${data.health.length} health warning${data.health.length === 1 ? "" : "s"}`);
              summaryEl.textContent = parts.join(" - ");
            } else {
              summaryEl.textContent = String((data && data.error) || "Tailscale unavailable");
            }
          }
          if (!listEl) return;
          listEl.replaceChildren();
          if (!ok) {
            listEl.appendChild(el("div", { class: "tailscaleEmpty muted", text: "No machine data available" }));
            return;
          }
          if (!machines.length) {
            listEl.appendChild(el("div", { class: "tailscaleEmpty muted", text: "No Tailscale machines found" }));
            return;
          }
          for (const machine of machines.slice(0, 80)) {
            const name = tailscaleMachineName(machine);
            const ips = machine && Array.isArray(machine.ips) ? machine.ips.filter(Boolean) : [];
            const dns = String((machine && machine.dns_name) || "").trim();
            const relay = String((machine && machine.relay) || "").trim();
            const os = String((machine && machine.os) || "").trim();
            const handshake = tailscaleRelativeTime(machine && machine.last_handshake_ts);
            const traffic = `${fmtBytes((machine && machine.rx_bytes) || 0)} down / ${fmtBytes((machine && machine.tx_bytes) || 0)} up`;
            const details = [];
            if (ips[0]) details.push(ips[0]);
            if (os) details.push(os);
            if (relay) details.push(`relay ${relay}`);
            if (handshake) details.push(`handshake ${handshake}`);
            const row = el("div", { class: `tailscaleMachine ${machine && machine.online ? "online" : "offline"}` }, [
              el("span", { class: "tailscaleDot" }),
              el("div", { class: "tailscaleMachineMain" }, [
                el("div", { class: "tailscaleMachineName", text: `${name}${machine && machine.is_self ? " (this)" : ""}` }),
                el("div", { class: "tailscaleMachineMeta", text: details.join(" - ") || dns || "No endpoint details" }),
                el("div", { class: "tailscaleMachineTraffic", text: traffic }),
              ]),
            ]);
            if (dns) row.title = dns;
            listEl.appendChild(row);
          }
        }

        async function loadTailscaleStatus({ immediate = false } = {}) {
          try {
            const data = await api("/api/tailscale/status");
            renderTailscaleStatus(data);
          } catch (e) {
            if (e && e.status === 401) return;
            renderTailscaleStatus({ ok: false, error: e && e.message ? e.message : "status request failed", machines: [] });
            if (immediate) setToast(`tailscale error: ${e && e.message ? e.message : "unknown error"}`);
          }
        }

        function renderTokenUsageToday(data) {
          const ok = !!(data && data.ok);
          const total = ok ? Number(data.total_tokens || 0) : 0;
          tokenUsageBtn.classList.toggle("active", ok && total > 0);
          tokenUsageBtn.classList.toggle("error", !ok);
          tokenUsageBtn.title = ok ? `Today tokens ${fmtTokenCount(total)}` : `Token usage unavailable: ${String((data && data.error) || "unknown error")}`;
          tokenUsageBtn.setAttribute("aria-label", tokenUsageBtn.title);
          const countEl = $("#tokenUsageCount");
          if (countEl) {
            countEl.textContent = ok ? fmtTokenCount(total, { compact: true }) : "!";
            countEl.style.display = "inline-flex";
          }
          const updatedEl = $("#tokenUsageUpdated");
          const summaryEl = $("#tokenUsageSummary");
          const gridEl = $("#tokenUsageGrid");
          const rootsEl = $("#tokenUsageRoots");
          const sessionsEl = $("#tokenUsageSessions");
          if (updatedEl) updatedEl.textContent = data && data.updated_ts ? tailscaleRelativeTime(data.updated_ts) : "";
          if (summaryEl) {
            if (ok) {
              const date = String(data.date || "today");
              const events = Number(data.token_events || 0);
              const files = Number(data.files || 0);
              summaryEl.textContent = `${date} - ${fmtTokenCount(total)} total - ${events} events - ${files} logs`;
            } else {
              summaryEl.textContent = String((data && data.error) || "Token usage unavailable");
            }
          }
          if (gridEl) {
            gridEl.replaceChildren();
            const rows = [
              ["Input", data && data.input_tokens],
              ["Cached", data && data.cached_input_tokens],
              ["Output", data && data.output_tokens],
              ["Reasoning", data && data.reasoning_output_tokens],
            ];
            for (const [label, value] of rows) {
              gridEl.appendChild(
                el("div", { class: "tokenUsageMetric" }, [
                  el("div", { class: "tokenUsageMetricLabel", text: label }),
                  el("div", { class: "tokenUsageMetricValue", text: ok ? fmtTokenCount(value) : "-" }),
                ])
              );
            }
          }
          if (rootsEl) {
            ...
            if (ok) {
              ...
              for (const root of roots) {
                rootsEl.appendChild(
                  el("div", { class: "tokenUsageRoot" }, [
                    el("span", { class: "tokenUsageRootName", text: String((root && root.label) || "root") }),
                    el("span", { class: "tokenUsageRootValue", text: `${fmtTokenCount(root && root.total_tokens)} tokens` }),
                  ])
                );
              }
            }
          ...
          if (!ok) {
            sessionsEl.appendChild(el("div", { class: "tokenUsageEmpty muted", text: "No token usage data available" }));
            return;
          }
          const sessions = Array.isArray(data.sessions) ? data.sessions : [];
          if (!sessions.length) {
            sessionsEl.appendChild(el("div", { class: "tokenUsageEmpty muted", text: "No token events today" }));
            return;
          }
          sessionsEl.appendChild(el("div", { class: "tokenUsageSectionTitle", text: "Top sessions" }));
          for (const row of sessions.slice(0, 12)) {
            const item = el("div", { class: "tokenUsageSession" }, [
              el("div", { class: "tokenUsageSessionMain" }, [
                el("div", { class: "tokenUsageSessionName", text: tokenUsageSessionName(row) }),
                el("div", { class: "tokenUsageSessionMeta", text: `${String((row && row.label) || "")} - ${Number((row && row.token_events) || 0)} events` }),
              ]),
              el("div", { class: "tokenUsageSessionTokens", text: fmtTokenCount(row && row.total_tokens, { compact: true }) }),
            ]);
            if (row && row.path) item.title = String(row.path);
            sessionsEl.appendChild(item);
          }
          const truncated = Number(data.truncated_sessions || 0);
          if (truncated > 0) sessionsEl.appendChild(el("div", { class: "tokenUsageMore muted", text: `+${truncated} more sessions` }));
        }

        async function loadTokenUsageToday({ immediate = false } = {}) {
          try {
            const data = await api("/api/token_usage/today");
            renderTokenUsageToday(data);
          } catch (e) {
            if (e && e.status === 401) return;
            renderTokenUsageToday({ ok: false, error: e && e.message ? e.message : "usage request failed" });
            if (immediate) setToast(`token usage error: ${e && e.message ? e.message : "unknown error"}`);
          }
        }
...
`;
        ...
        document.addEventListener("keydown", (e) => {
          ...
          if (tokenUsagePanel.style.display === "block") {
            hideTokenUsagePanel();
            return;
          }
          if (tailscalePanel.style.display === "block") {
            hideTailscalePanel();
            return;
          }
          ...
          if (splitChoice.style.display === "flex") hideSplitChoice();
        ...
        form.onsubmit = async (e) => {
          ...
          if (ok && $("#msg").value === raw) clearComposer();
	        ...
	        (async () => {
	          if (localStorage.getItem(paneStorageKey("codexweb.sidebarCollapsed")) === "1") setSidebarCollapsed(true);
	          if (localStorage.getItem(paneStorageKey("codexweb.sidebarOpen")) === "1") setSidebarOpen(true);
              ...
              const hashed = sessionIdFromHash();
	            const remembered = localStorage.getItem(paneStorageKey("codexweb.selected"));
      ...
      (async function boot() {
        try {
          ...
          if (e && e.status === 401) {
            renderLogin(splitShellMode ? renderSplitShell : renderApp);
            return;
          }
