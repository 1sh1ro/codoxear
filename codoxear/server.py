...
TAILSCALE_STATUS_MAX_PEERS = 80
TOKEN_USAGE_MAX_SESSION_ROWS = 24
...
def _parse_tailscale_time(value: Any) -> float | None:
    if not isinstance(value, str):
        return None
    raw = value.strip()
    if not raw or raw.startswith("0001-01-01T"):
        return None
    s = raw
    if s.endswith("Z"):
        s = s[:-1] + "+00:00"
...
def _tailscale_peer_summary(peer: Any, *, is_self: bool = False) -> dict[str, Any] | None:
    if not isinstance(peer, dict):
        return None
    hostname = str(peer.get("HostName") or "").strip()
    dns_name = str(peer.get("DNSName") or "").strip().rstrip(".")
    ips_raw = peer.get("TailscaleIPs")
    ips = [str(x) for x in ips_raw if isinstance(x, str) and x.strip()] if isinstance(ips_raw, list) else []
    if not hostname and not dns_name and not ips:
        return None
    summary: dict[str, Any] = {
        "hostname": hostname,
        "dns_name": dns_name,
        "os": str(peer.get("OS") or "").strip(),
        "ips": ips[:4],
        "online": bool(peer.get("Online")),
        "active": bool(peer.get("Active")),
        "relay": str(peer.get("Relay") or "").strip(),
        "rx_bytes": int(peer.get("RxBytes") or 0),
        "tx_bytes": int(peer.get("TxBytes") or 0),
        "last_seen_ts": _parse_tailscale_time(peer.get("LastSeen")),
        "last_handshake_ts": _parse_tailscale_time(peer.get("LastHandshake")),
        "is_self": bool(is_self),
    }
    return summary


def _tailscale_status_snapshot() -> dict[str, Any]:
    exe = shutil.which("tailscale")
    if not exe:
        return {"ok": False, "error": "tailscale command not found", "available": False, "machines": []}
    try:
        proc = subprocess.run(
            [exe, "status", "--json"],
            capture_output=True,
        ...
        return {"ok": False, "error": "tailscale status timed out", "available": True, "machines": []}
    except OSError as e:
        return {"ok": False, "error": str(e), "available": False, "machines": []}
    if proc.returncode != 0:
        msg = (proc.stderr or proc.stdout or f"tailscale exited with status {proc.returncode}").strip()
        return {"ok": False, "error": msg, "available": True, "machines": []}
    try:
        data = json.loads(proc.stdout or "{}")
    except json.JSONDecodeError as e:
        return {"ok": False, "error": f"invalid tailscale json: {e}", "available": True, "machines": []}
    if not isinstance(data, dict):
        return {"ok": False, "error": "invalid tailscale json", "available": True, "machines": []}
    machines: list[dict[str, Any]] = []
    self_summary = _tailscale_peer_summary(data.get("Self"), is_self=True)
    if self_summary:
        machines.append(self_summary)
    peers_raw = data.get("Peer")
    if isinstance(peers_raw, dict):
        peers = [_tailscale_peer_summary(peer) for peer in peers_raw.values()]
        peers = [peer for peer in peers if peer is not None]
        peers.sort(
            key=lambda x: (
                not bool(x.get("online")),
                str(x.get("hostname") or x.get("dns_name") or "").lower(),
            )
        )
        machines.extend(peers[:TAILSCALE_STATUS_MAX_PEERS])
    online = sum(1 for machine in machines if machine.get("online"))
    return {
        "ok": True,
        "available": True,
        "backend_state": str(data.get("BackendState") or "").strip(),
        "tailnet": str((data.get("CurrentTailnet") or {}).get("Name") or "").strip() if isinstance(data.get("CurrentTailnet"), dict) else "",
        "magic_dns_suffix": str(data.get("MagicDNSSuffix") or "").strip(),
        "self_ips": [str(x) for x in data.get("TailscaleIPs", []) if isinstance(x, str)][:4] if isinstance(data.get("TailscaleIPs"), list) else [],
        "health": [str(x) for x in data.get("Health", []) if isinstance(x, str)] if isinstance(data.get("Health"), list) else [],
        "online_count": online,
...
TOKEN_USAGE_FIELDS = ("input_tokens", "cached_input_tokens", "output_tokens", "reasoning_output_tokens", "total_tokens")


def _parse_log_timestamp(value: Any) -> datetime.datetime | None:
    if not isinstance(value, str):
        return None
    raw = value.strip()
    if not raw:
    ...
    if raw.endswith("Z"):
        raw = raw[:-1] + "+00:00"
    m = re.match(r"^(.*T\d{2}:\d{2}:\d{2})\.(\d+)([+-]\d{2}:\d{2})$", raw)
    if m:
        raw = f"{m.group(1)}.{m.group(2)[:6].ljust(6, '0')}{m.group(3)}"
    try:
...
def _empty_token_usage_counts() -> dict[str, int]:
    return {field: 0 for field in TOKEN_USAGE_FIELDS}


def _add_token_usage_counts(dst: dict[str, int], usage: dict[str, Any]) -> None:
    for field in TOKEN_USAGE_FIELDS:
        value = usage.get(field)
        if isinstance(value, int) and value >= 0:
            dst[field] = int(dst.get(field, 0)) + value


def _token_usage_from_log_obj(obj: dict[str, Any]) -> dict[str, int] | None:
    if obj.get("type") == "event_msg":
        ...
        if not isinstance(total, int):
            return None
        out = _empty_token_usage_counts()
        _add_token_usage_counts(out, last)
        return out
...
def _iter_token_usage_log_roots() -> list[tuple[str, Path]]:
    roots: list[tuple[str, Path]] = []
    seen: set[str] = set()
    for label, path in (
        ("codoxear", CODEX_SESSIONS_DIR),
...
def _today_token_usage_snapshot(now: datetime.datetime | None = None) -> dict[str, Any]:
    ...
    files_scanned = 0
    token_events = 0
    for label, root in _iter_token_usage_log_roots():
        root_total = _empty_token_usage_counts()
        root_files = 0
        root_events = 0
        if not root.exists():
            by_root.append({"label": label, "path": str(root), "exists": False, "files": 0, "token_events": 0, **root_total})
            continue
        for path in sorted(root.rglob("*.jsonl")):
            file_total = _empty_token_usage_counts()
            file_events = 0
            try:
                with path.open("r", encoding="utf-8") as fh:
                    for line in fh:
                        ...
                        if not isinstance(obj, dict):
                            continue
                        ts = _parse_log_timestamp(obj.get("timestamp"))
                        if ts is None or not (start <= ts < end):
                            continue
                        usage = _token_usage_from_log_obj(obj)
                        if usage is None:
                            continue
                        _add_token_usage_counts(total, usage)
                        _add_token_usage_counts(root_total, usage)
                        _add_token_usage_counts(file_total, usage)
                        token_events += 1
                        root_events += 1
                        file_events += 1
            except OSError:
                ...
                session_rows.append(
                    {
                        ...
                        "path": str(path),
                        "name": path.name,
                        "token_events": file_events,
                        **file_total,
                    }
                )
        by_root.append({"label": label, "path": str(root), "exists": True, "files": root_files, "token_events": root_events, **root_total})
    session_rows.sort(key=lambda row: int(row.get("total_tokens") or 0), reverse=True)
    return {
        "ok": True,
        ...
        **total,
        "roots": by_root,
        "sessions": session_rows[:TOKEN_USAGE_MAX_SESSION_ROWS],
        "session_count": len(session_rows),
        "truncated_sessions": max(0, len(session_rows) - TOKEN_USAGE_MAX_SESSION_ROWS),
    }
        ...
        try:
            ...
            if path == "/api/tailscale/status":
                if not _require_auth(self):
                    self._unauthorized()
                    return
                _json_response(self, 200, _tailscale_status_snapshot())
                return

            if path == "/api/token_usage/today":
                if not _require_auth(self):
                    self._unauthorized()
                    return
                _json_response(self, 200, _today_token_usage_snapshot())
                return
